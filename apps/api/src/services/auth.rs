...
use crate::{auth, error::AppError, jwt, repos::users as users_repo};
...
pub async fn login(
    ...
    let row = users_repo::find_active_user_for_login(pool, &input.email)
        .await
        .map_err(|e| {
    ...
    let ok = auth::verify_password(&input.password, &hash).map_err(|e| {
...
pub async fn signup(
    ...
    let user_id = match users_repo::insert_signup_user(pool, &email, name, &password_hash).await {
        Ok(id) => id,
        Err(e) => {
...
pub async fn refresh(
    ...
    let user = users_repo::find_active_user_by_id(pool, user_id)
        .await
        .map_err(|e| {
